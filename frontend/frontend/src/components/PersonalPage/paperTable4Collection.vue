...
<template>
  ...
  <el-table fit ref="tableRef" row-key="title" :data="tableData">
    ...
    >
      <template #default="scope">
        <div>
          ...
          <br />
          <span class="ellipsis" v-if="scope.row.authors!=null && scope.row.authors.length!=0 && scope.row.authors !=', ' && scope.row.authors !='null'" style="font-size: 14px;
                       line-height: 1.0;
                       font-weight: lighter">
            {{ scope.row.authors }}
          </span>
    ...
    <el-table-column v-if="getUserId() == String(route.params.id)" width="100">
      <template #default="scope">
...
function handleDelete(row) {
  console.log("row",row)
  deleteCollection(Number(getUserId()), row.achievementId, row.fid).then(() => {
    tableData.value = tableData.value.filter(item => item.achievementId !== row.achievementId);
  })
}
